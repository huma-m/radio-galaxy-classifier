...
class BinaryCRUMB(CRUMB):

    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)

        targets = np.array(self.targets)
        complete = np.array(self.complete_labels)
        mask = targets != 2

        self.data = self.data[mask]
        self.targets = targets[mask].tolist()
        self.complete_labels = complete[mask].tolist()

class ConfidentBinaryCRUMB(CRUMB):
    ...
    def __init__(self, *args, **kwargs):
        ...
        binary_mask = targets != 2

        mask = confident_mask & binary_mask
        self.data = self.data[mask]
        self.targets = targets[mask].tolist()
        self.complete_labels = complete[mask].tolist()
...
def is_confident(row):
    ...
    if mb_label != -1:
        return mb_label in [0, 1, 2, 5, 6]

    # Not in MiraBest
    fd_label = row[1]   # FR-DEEP: 0=FRI, 1=FRII
    at_label = row[2]   # AT17: 0=FRI, 1=FRII, 2=Bent

    present_labels = []
    if fd_label != -1:
        present_labels.append(fd_label)
    if at_label != -1 and at_label != 2:   # exclude Bent from confident fallback
        present_labels.append(at_label)

    if len(present_labels) == 0:
        return False
    return len(np.unique(present_labels)) == 1